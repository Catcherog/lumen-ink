...
- [x] 访问 `https://glm-image-editor.vercel.app/api/auth` 时，正确密码返回 200 + token，错误密码返回 401，不再出现 500
...
- [x] Vercel Runtime Logs 中 serverless function 启动无模块找不到或类型错误（通过线上 `/api/auth` 正常响应确认）
- [x] 线上 `https://glm-image-editor.vercel.app/` 打开无白屏，登录后正常进入编辑器主界面（通过线上页面可见文本 "请输入密码以访问" 与 `/api/auth` 200 响应确认）
