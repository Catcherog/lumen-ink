...
| `docs/lumen-v2/prompts/NEW-WINDOW-GPT.md` | 固定模板（无需替换占位符），告知 GPT 读取本地文件恢复上下文 |
...
- 本规则在 `_trae-execution.md` 之后执行（见 `_core.md` 规则间优先级第 6、7 条）。
- 涉及 Lumen V2 协作任务时：
  - **状态机与落盘位置**以本规则为准（`_trae-execution.md` 不重复定义）。
  - **通用执行流程**（Preflight、完成包、Codex 协作、EXECUTION_CONFLICT 格式）以 `_trae-execution.md` 为准。
